...
- 进度日志展示去掉 query/hash 的接口基址，并同时显示当前数据库账号邮箱；这只是脱敏展示，实际 GET 仍使用数据库中的完整 URL。
...
- [x] 进度窗口能明确显示“接口基址 + 当前账号邮箱”，不会把脱敏后的基址误认为共享收件箱。
...
- 2026-07-15：补充 Plus/验活进度事件回归测试，验证日志中的账号邮箱与本次请求账号一致。
