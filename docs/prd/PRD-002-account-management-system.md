...
- **邮件获取**:
  ...
  - 邮件获取结果以列表展示在页面下方，不设置内部纵向滚动，内容随页面自然向下撑开。
...
- **一键补号 / 批量补号**: 
  ...
  - 补号失败后，状态流转为 `failed`，计数值不累加，将错误信息记录在最后操作信息中，并累加连续补号失败次数。
  - 同一补号账号连续补号失败达到 5 次时，系统自动触发熔断：账号状态流转为 `banned`，写入熔断时间和原因，不再进入 CPA 自动补号队列，并创建站内通知提醒管理员。
  - 补号成功后，连续补号失败次数与熔断字段必须清零。
  - 管理员可在补号管理页对已触发熔断的账号执行“解除熔断”：账号状态改为 `pending`，连续失败次数清零，熔断时间和原因清空，状态备注写入“管理员手动解除熔断”。
...
- CPA 自动补号连续失败触发本地账号熔断后，应通过站内通知告诉管理员具体账号、连续失败次数和已自动标记 `banned` 的结果。
...
#### 功能细则
- 后台顶部铃铛必须显示真实未读通知数量；无未读通知时隐藏数字角标。
- 点击铃铛后展示最近通知列表。
- 通知列表支持将单条未读通知标记为已读。
- CPA 自动补号连续失败 5 次触发熔断时，必须创建一条未读站内通知。

---

### 2.5 浏览器自动化适配 (RoxyBrowser & Playwright)
...
- OAuth 自动化状态机应覆盖：
  ...
  2. 可选 OpenAI 密码页：识别 `Enter your password` / `Password`，点击 `Log in with a one-time code`，再交回后续状态机。
  3. 邮箱验证码页：调用验证码接口获取 6 位验证码并提交。
  4. 可选添加手机号页：从补号账号手机号或运行参数中读取手机号，填写 `Phone number` 并点击 `Continue`。
  5. 可选手机验证方式页：选择 `Text Message` 并继续。
  6. 可选手机验证码页：从 SMS API 响应中提取 6 位验证码并提交。
  7. Codex/ChatGPT 授权确认页：点击 `Continue`。
  8. OAuth callback：捕获 `code/state`，校验 state，并使用授权码换取 token bundle。
...
- 自动化日志应记录邮箱提交后的 next stage、密码页识别、one-time code 后 next stage、异常页面重试次数和重试耗尽；日志不得输出验证码、Cookie 或 token 明文。
- 页面步骤失败时默认截图到 `debug_image/`，截图文件名不得包含邮箱、验证码、API key 或 URL 等敏感信息。
- 邮箱验证码和手机验证码获取应支持轮询；验证码为空时只等待下一轮，不填写、不点击提交。
- 邮箱提交后进入密码页时，应点击 `Log in with a one-time code` 并继续邮箱验证码、Codex 授权或 callback 流程；密码页只读邮箱输入框不得被误判为邮箱登录页。
- 点击 one-time code 后短暂停留密码页时，不得把当前 `openai-password` 阶段当作有效后续阶段。
- 邮箱提交后进入未知页面时，应重新导航本次 OAuth target URL 并重新提交邮箱；默认最多重试 3 次，耗尽后抛出可识别错误。
...
- [x] 邮件获取结果列表不出现内部上下滚动条，内容随页面自然撑开。
...
- [x] Roxy OAuth 自动化可识别 OpenAI 密码页并点击 one-time code，随后继续验证码、Codex 授权或 callback 流程。
- [x] 邮箱提交后进入未知页面时，自动化会回到本次 OAuth target URL 重试，并在耗尽后抛出清晰错误。
...
- [x] CPA 自动补号连续失败 5 次会自动熔断账号为 `banned`，并写入熔断时间和原因。
- [x] 管理员可通过补号管理页解除账号熔断，解除后账号回到 `pending` 并清空连续失败和熔断字段。
