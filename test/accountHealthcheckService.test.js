...
test('runBannedEmailHealthcheck marks eligible matching accounts as banned', async () => {
  ...
  const plus = replacementAccounts.createAccount({ email: 'receiver+plus@gmail.com', status: 'plus_active' });
  const sold = replacementAccounts.createAccount({ email: 'sold@icloud.com', status: 'sold' });
  ...
  assert.deepEqual(
    ...
    [
      ['receiver@gmail.com', 'inbox', 5, 'receiver+plus@gmail.com'],
      ['receiver@gmail.com', 'inbox', 5, 'sold@icloud.com'],
    ],
  );
});
...
test('runBannedEmailHealthcheck reports clean and failed accounts without changing status', async () => {
  ...
  const clean = replacementAccounts.createAccount({ email: 'receiver+clean@gmail.com', status: 'for_sale' });
  const failed = replacementAccounts.createAccount({ email: 'receiver+failed@gmail.com', status: 'cpa_mounted' });
  ...
  const result = await runBannedEmailHealthcheck({
    ...
    mailService: {
      async fetchMessages(account, options) {
        ...
        throw Object.assign(new Error('IMAP temporary failure'), { code: 'IMAP_ERROR' });
      },
    },
  });
