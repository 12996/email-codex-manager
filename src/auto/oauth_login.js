...
// 将代理 URL 转换为 Playwright 浏览器启动所需的代理配置。
...
// 构建 Axios 请求使用的代理 Agent 配置，支持 HTTP/HTTPS/SOCKS 代理。
...
// 转义字符串中的正则特殊字符，便于安全拼接正则表达式。
function escapeRegex(str) {
    return String(str || '').replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
}

// 规范化云邮箱地址，补齐默认域名并修正重复域名。
...
// 按当前邮箱来源轮询获取 OpenAI 登录验证码，并支持排除旧验证码。
...
// 等待指定毫秒数，用于模拟人工节奏或轮询间隔。
const sleep = (ms) => new Promise(res => setTimeout(res, ms));

// 在登录流程异常时保存当前页面截图，便于后续排查。
...
// 尝试从 OpenAI 超时页恢复，并重新提交邮箱回到验证码流程。
...
// 执行完整 OpenAI OAuth 登录流程，获取 token 并导出账号协议文件。
