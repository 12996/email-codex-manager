...
def generate_sentinel_token(
    ...
    cores = int(fingerprint.get("hardwareConcurrency") or 32)
    language_value = fingerprint.get("language") or "ja-JP"
    languages_value = fingerprint.get("languages") or ["ja-JP", "ja", "en-US", "en"]
    language = str(language_value)
    languages = (
        ",".join(str(item) for item in languages_value)
        if isinstance(languages_value, (list, tuple))
        else str(languages_value)
    )
    ua = user_agent or fingerprint.get("userAgent") or USER_AGENT
