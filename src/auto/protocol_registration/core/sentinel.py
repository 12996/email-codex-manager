...
def generate_fingerprint_data(
    ...
    user_agent = str(fingerprint.get("userAgent") or USER_AGENT)
    language = str(fingerprint.get("language") or "ja-JP")
    languages_value = fingerprint.get("languages") or ["ja-JP", "ja", "en-US", "en"]
    if isinstance(languages_value, (list, tuple)):
        languages = ",".join(str(item) for item in languages_value)
    else:
        languages = str(languages_value)
    hardware_concurrency = int(fingerprint.get("hardwareConcurrency") or 32)
    ...
    config = [
        ...
        languages,                        # [8] navigator.languages
...
def generate_requirements_token(device_id: str, fingerprint: dict | None = None) -> str:
    """
