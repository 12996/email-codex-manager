    ...
    for attempt in range(1, _FOLLOW_AUTH_MAX_ATTEMPTS + 1):
        try:
            ...
            if getattr(session, "uses_roxy_cdp", False):
                resp = session.navigate(authorize_url, headers=headers, allow_redirects=True)
            else:
...
def request_sentinel_token(session: BrowserSession, flow: str) -> dict:
    ...
    """
    ensure_fingerprint = getattr(session, "ensure_fingerprint", None)
    if callable(ensure_fingerprint):
        ensure_fingerprint()

    browser_sentinel_headers = getattr(session, "sentinel_headers", None)
    if getattr(session, "uses_roxy_cdp", False) and callable(browser_sentinel_headers):
        generated = browser_sentinel_headers(flow)
        session._pending_sentinel_headers = {"flow": flow, "headers": generated}
        try:
            generated_header = json.loads(generated.get("header") or "{}")
        except (TypeError, ValueError):
            generated_header = {}
        return {
            "token": generated_header.get("c", ""),
            "persona": "roxy-page-sdk",
            "_roxy_cdp_headers": generated,
            "turnstile": {"required": bool(generated_header.get("t"))},
            "proofofwork": {"required": False},
            "so": {"required": bool(generated.get("so_header"))},
        }
    ...
    p = generate_requirements_token(
        session.device_id,
        fingerprint=getattr(session, "roxy_fingerprint", None),
    )
...
def build_sentinel_header(session: BrowserSession, sentinel_resp: dict, flow: str) -> tuple:
    ...
    实现策略：CDP 模式直接在 Roxy 页面中运行 Sentinel SDK，并调用
    sessionObserverToken() 取得 SO；默认 curl_cffi 模式继续使用本地 sentinel-runner.js。
    ...
    browser_sentinel_headers = getattr(session, "sentinel_headers", None)
    if getattr(session, "uses_roxy_cdp", False) and callable(browser_sentinel_headers):
        pending = getattr(session, "_pending_sentinel_headers", None)
        if pending and pending.get("flow") == flow:
            browser_result = pending["headers"]
            session._pending_sentinel_headers = None
        else:
            browser_result = browser_sentinel_headers(flow)
        header_value = browser_result["header"]
        so_header = browser_result.get("so_header") or None
    else:
